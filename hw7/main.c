...
//
// This file has some code and many comments to 
...
char arr[256];
sprintf(arr,"\tthis is the pipe message from the parent with PID %d\n",pid);
...
//
 	switch (pid = fork()){
		...
		printf("cant make a fork! ");
		exit(-1);
	default:
		write(msg_pipe[1],arr,100);
		exit(0);
	case 0:
