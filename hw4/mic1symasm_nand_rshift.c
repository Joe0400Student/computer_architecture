...
#define         STR       29
#define         MULT	  30
#define         RSHIFT	  31 //added mult and div here
#define		DIV	  32
...
// dump the symbol table after the binary code
	...
	unlink("/tmp/passone");
        ...
        switch(tok){
          ...
          case HALT: fprintf(p1,"%d  1111111111111111\n",pc);
                   break;

          case MULT: if((tok=yylex()) != INTEG){ // ADDED MULT AND DIV HERE
			fprintf(stderr, "Bad operand after MUL is %s\n",yytext);
			exit(1);
		  }
		  str_6(yytext);
